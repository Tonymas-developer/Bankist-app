...
const deposit = movements.filter((e) => {
  return e > 0;
});
console.log(movements);
console.log(deposit);

const withdrawal = movements.filter((e) => {
  return e < 0;
});
console.log(withdrawal);

let total = 0;
for (const tot of movements) {
  total += tot;
}
console.log(total);
...
// NUMBERS
const maximum = [1, 2, 3, 4, 44];
console.log(Math.max(1, 2, 23, 5));
...
console.log((2.722619).toFixed(3));
...
labelBalance.addEventListener("click", (e) => {
  e.preventDefault();
  const doc = document.querySelectorAll(".movements__row");
  doc.forEach((row, i) => {
    if (i % 2 === 0) {
      row.style.backgroundColor = "red";
    }
